...
export const OppositeGame = () => {
  ...
  useEffect(() => { validOppositesRef.current = validOpposites; }, [validOpposites]);
  ...
  const loadNewWord = async () => {
    ...
    speechManager.speak(`What is the opposite of ${newRoot}?`);

    try {
      // Fetch antonyms (opposites)
      const res = await fetch(`https://api.datamuse.com/words?rel_ant=${newRoot}&max=20`);
      ...
      // Hardcode fallbacks if datamuse misses some common ones for kids
      const hardcodedFallbacks: Record<string, string[]> = {
        'HOT': ['COLD'], 'UP': ['DOWN'], 'DAY': ['NIGHT', 'DARK'],
        'GOOD': ['BAD'], 'BIG': ['SMALL', 'LITTLE'], 'FAST': ['SLOW'],
        'HAPPY': ['SAD', 'MAD'], 'TALL': ['SHORT'], 'WET': ['DRY'], 'OPEN': ['CLOSE', 'SHUT']
      };

      if (hardcodedFallbacks[newRoot]) {
        opposites = [...new Set([...opposites, ...hardcodedFallbacks[newRoot]])];
      }

      setValidOpposites(opposites.length > 0 ? opposites : ['YES']);
    } catch (e) {
      console.error("Failed to fetch opposites", e);
      setValidOpposites(['COLD', 'DOWN', 'NIGHT', 'BAD', 'SMALL', 'SLOW']);
    }
    setIsLoading(false);
  };
  ...
  usePixiApp({
    ...
    onInit: (app: PIXI.Application) => {
      ...
      const onKeyDown = (e: KeyboardEvent) => {
        ...
        if (!rootWordRef.current || validOppositesRef.current.length === 0) return;

        if (validOppositesRef.current.includes(currentAttemptRef.current)) return;
    ...
    <div className="relative w-full h-full overflow-hidden flex flex-col items-center">
      ...
      <div className="absolute top-6 z-10 flex flex-col items-center gap-2 max-w-[90vw]">
        <div className="flex gap-2 p-3 bg-white/10 backdrop-blur-md rounded-3xl border border-white/20 shadow-xl items-center text-white font-bold text-xl px-8">
          <FlipHorizontal className="mr-2" /> Opposite Day
        </div>
      </div>
      ...
      <div className="absolute top-40 z-10 flex flex-col items-center mt-8 w-full px-4">
        {rootWord && (
          <div className="mb-4 text-center">
            <div className="text-xl text-orange-200 font-bold mb-2 uppercase tracking-widest">What is the opposite of...</div>
            <div className="px-12 py-4 bg-orange-500 text-white rounded-[40px] text-6xl md:text-8xl font-black border-4 border-orange-300 shadow-[0_0_40px_rgba(249,115,22,0.5)] animate-bounce font-mono">
              {rootWord}
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3 mt-8 items-center justify-center flex-wrap max-w-4xl min-h-[120px]">
          {isLoading ? (
            ...
            Array.from({ length: Math.min(7, currentAttempt.length + 1) }).map((_, i) => {
              const char = currentAttempt[i];
              const isCurrent = i === currentAttempt.length;
              const isDone = validOpposites.includes(currentAttempt);
              ...
              return (
                <div key={i} className={cn(
                  ...
                  isDone ? "bg-emerald-400 text-white border-emerald-300 scale-110 opacity-100 shadow-[0_0_50px_rgba(52,211,153,0.8)]" :
                    char ? "bg-orange-400 text-white border-orange-300 scale-100" :
